exports.view = function(req, res){
	...
	pool.getConnection(function(err, connection) {
		connection.query('SELECT id_rbt, rbt.id_user, firstname, lastname, photo_user, facebook_id,'+
			...
			+' FROM rbt inner join (select id_rbt_sun, count(id_rbt_sun) as sun from sunshine group by id_rbt_sun) as sun_count on id_rbt_sun = id_rbt inner join user on rbt.id_user = user.id_user where rbt.id_user = '+ id_user +' ORDER BY created_at DESC;', function(err, rows_rbt) {
	    	res.render('friend', {
				rbt: rows_rbt
			});
	        connection.release();
